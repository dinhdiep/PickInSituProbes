...
our $minKmerOffTarget_max_occ = 500;

our $path_to_melting = "~/softwares/MELTING5.1.1/executable";
our $path_to_bowtie = "~/softwares/bowtie-1.1.1";

# Bowtie mappings
our $bowtie_filter_cmd = "$path_to_bowtie/bowtie -p 8 -f -a --strata --best Data/GRCh38 $job_name.kmer.fa > $job_name.kmer.bowtie.txt 2>> $job_name.kmer.log";
our $bowtie_find_off_target_cmd = "$path_to_bowtie/bowtie -p 8 -f -a -l 10 --best -n 2 Data/GRCh38 $job_name.oligo.fa > $job_name.bowtie.txt 2> $job_name.oligo.log";
